...
public class CMDBManager {
  ...
  public static void init(CMInfo cmInfo) {
    ...
    url += "?useUnicode=true&characterEncoding=euckr&serverTimezone=UTC";  // for using Korean character
    dbInfo.setDBURL(url);
  ...
  public static int queryUpdateUser(String name, String fieldName, String value, CMInfo cmInfo) {
    ...
    if (fieldName.equals("password")) {
      ...
      strQuery =
          "update user_table set " + fieldName + "='" + value + "' where userName='" + name + "';";
    }
    int ret = sendUpdateQuery(strQuery, cmInfo);
  ...
  public static CMSNSContentList queryGetSNSContent(String strRequester, String strWriter,
      int index, int num, CMInfo cmInfo) {
    String strQuery = null;
    ResultSet rs = null;
    ...
    } else if (strWriter.equals("CM_MY_FRIEND")) // retrieve all content of friends of the requester
    {
      strQuery = "select * from sns_content_table where (levelOfDisclosure<=1 AND "
        ...
        .equals("CM_BI_FRIEND")) // retrieve all content of bi-friends of the requester
    {
    ...
    if (CMInfo._CM_DEBUG) {
      System.out.println(
          "CCMDBManager.queryGetSNSContents(), end requester(" + strRequester + "), writer("
  ...
  public static void showResultSNSContent(ResultSet rs) {
    ...
    try {
      while (rs.next()) {
        ...
        System.out.format(
            "%-3s # Attached Files: %d, ID of replied content: %d, Level of Disclosure: %d%n",
  ...
  public static int queryInsertSNSAttachedFile(int nContentID, String strFilePath,
      String strFileName, CMInfo cmInfo) {
    int ret = -1;
    ...
    if (ret == -1) {
      System.out.println(
          "CMDBManager.queryInsertSNSAttachedFile(), error for content (" + nContentID + ").");
      return ret;
    }
  ...
  public static int queryUpdateSNSAttachedFile(int seqNum, String fieldName, String value,
    ...
    int ret = -1;
    String strQuery =
        "update attached_file_table set " + fieldName + "='" + value + "' where seqNum=" + seqNum
            + ";";
    ret = sendUpdateQuery(strQuery, cmInfo);
    if (ret == -1) {
      System.out
          .println("CMDBManager.queryUpdateSNSAttachedFile(), error for seqNum(" + seqNum + ").");
      return ret;
    }
  ...
  public static int queryDeleteSNSAttachedFile(int seqNum, CMInfo cmInfo) {
    ...
    if (ret == -1) {
      System.out
          .println("CMDBManager.queryDeleteSNSAttachedFile(), error for seqNum(" + seqNum + ").");
      return ret;
    }
  ...
  public static void showResultSNSAttachedFile(ResultSet rs) {
    ...
    try {
      while (rs.next()) {
        ...
        System.out.println(
            "seqNum: " + id + ", contentID: " + nContentID + ", path:" + strFilePath + ", name:"
                + strFileName);
      }
    } catch (SQLException e) {
      // TODO Auto-generated catch block
  ...
  public static ArrayList<String> queryGetRequestersList(String strUserName, CMInfo cmInfo) {
    ...
    if (CMInfo._CM_DEBUG) {
      System.out
          .println("CMDBManager.queryGetRequestersList(), end for user(" + strUserName + ").");
    }
  ...
  public static int queryInsertAccessHistory(String strUserName, Calendar date,
    ...
    String strQuery =
        "insert into sns_attach_access_history_table (userName, date, writerName, accessCount) "
            + "values ('" + strUserName + "', '" + strDate + "', '" + strWriterName + "', "
            + nAccessCount + ");";
    int ret = sendUpdateQuery(strQuery, cmInfo);
  ...
  // not yet called
  public static int queryDeleteAccessHistory(String strUserName, Calendar date,
      String strWriterName,
      CMInfo cmInfo) {
    int ret = -1;
    SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
    ...
    String strQuery =
        "delete from sns_attach_access_history_table where userName='" + strUserName + "' and "
  ...
  public static int queryUpdateAccessCount(String strUserName, Calendar date,
    ...
    String strQuery = "update sns_attach_access_history_table set accessCount=" + nAccessCount
        + " where userName='"
  ...
  public static CMSNSAttachAccessHistoryList queryGetAccessHistory(String strUserName,
    ...
    try {
      while (rs != null && rs.next()) {
        ...
        tempHistory = new CMSNSAttachAccessHistory(strUserName, date, strHistoryWriterName,
            nAccessCount);
